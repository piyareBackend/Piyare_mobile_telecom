...
final class SecureStore {
    ...
    private static final String ALIAS="pmt_sms_bridge_key_v4";
    ...
    static void putRefreshToken(Context c,String v)throws Exception{put(c,KEY_REFRESH,v);}
    static String getRefreshToken(Context c)throws Exception{return get(c,KEY_REFRESH);}
    static void clearTokenOnly(Context c){c.getSharedPreferences(PREFS,Context.MODE_PRIVATE).edit().remove(KEY_TOKEN).apply();}
    static void clearAuth(Context c){c.getSharedPreferences(PREFS,Context.MODE_PRIVATE).edit().remove(KEY_TOKEN).remove(KEY_REFRESH).apply();}
    ...
    private static void putInternal(Context c,String key,String value)throws Exception{
        ...
        byte[] iv=cipher.getIV();if(iv==null||iv.length==0)throw new Exception("Keystore did not provide an IV");
    ...
    private static String get(Context c,String key)throws Exception{
        String packed=c.getSharedPreferences(PREFS,Context.MODE_PRIVATE).getString(key,"");if(packed.isEmpty())return "";
        String[] parts=packed.split("\\.",2);if(parts.length!=2)return "";
        try{byte[] iv=Base64.decode(parts[0],Base64.NO_WRAP);if(iv.length==0)throw new Exception("Invalid stored IV");Cipher cipher=Cipher.getInstance("AES/GCM/NoPadding");cipher.init(Cipher.DECRYPT_MODE,getOrCreateKey(),new GCMParameterSpec(GCM_TAG_BITS,iv));return new String(cipher.doFinal(Base64.decode(parts[1],Base64.NO_WRAP)),StandardCharsets.UTF_8);}catch(Exception bad){reset(c);return "";}
    }
    private static SecretKey getOrCreateKey()throws Exception{
        KeyStore ks=KeyStore.getInstance(STORE);ks.load(null);
        if(ks.containsAlias(ALIAS)){KeyStore.Entry e=ks.getEntry(ALIAS,null);if(e instanceof KeyStore.SecretKeyEntry)return((KeyStore.SecretKeyEntry)e).getSecretKey();try{ks.deleteEntry(ALIAS);}catch(Exception ignored){}}
        KeyGenerator g=KeyGenerator.getInstance(KeyProperties.KEY_ALGORITHM_AES,STORE);
        g.init(new KeyGenParameterSpec.Builder(ALIAS,KeyProperties.PURPOSE_ENCRYPT|KeyProperties.PURPOSE_DECRYPT).setBlockModes(KeyProperties.BLOCK_MODE_GCM).setEncryptionPaddings(KeyProperties.ENCRYPTION_PADDING_NONE).setKeySize(256).setRandomizedEncryptionRequired(true).build());
