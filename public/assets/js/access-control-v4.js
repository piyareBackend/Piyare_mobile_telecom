...
(function(){
  ...
  function isAdminPageHref(h){var raw=String(h||'').trim();if(!raw||/^(https?:|mailto:|tel:|javascript:|#)/i.test(raw))return false;if(/^\/admin\//i.test(raw)||/^admin\//i.test(raw))return true;return /^[^/]+\.html(?:[?#].*)?$/i.test(raw)}
  ...
  function filterNavigation(u){var p=perms(u),owner=String(u.role)==='Owner';document.documentElement.dataset.pmtRole=String(u.role||'');document.querySelectorAll('a[href]').forEach(function(a){var href=String(a.getAttribute('href')||''),name=pageName(href),need=PAGE[name];if(need){if(!owner&&!ok(p,need))hide(a);return}if(!owner&&isAdminPageHref(href)&&name!=='index')hide(a)});document.querySelectorAll('[data-permission],[data-pmt-permission],[data-page-permission]').forEach(function(el){var n=el.getAttribute('data-permission')||el.getAttribute('data-pmt-permission')||el.getAttribute('data-page-permission');if(n&&!owner&&!ok(p,n))hide(el)});}
  function ensureObserver(u){if(observer||!document.body)return;observer=new MutationObserver(function(records){if(scheduled)return;for(var i=0;i<records.length;i++)if(records[i].addedNodes&&records[i].addedNodes.length){scheduled=true;requestAnimationFrame(function(){scheduled=false;filterNavigation(u)});break}});observer.observe(document.body,{childList:true,subtree:true})}
  function deny(){document.documentElement.style.visibility='visible';location.replace('/admin/login.html?access=denied')}
  function reveal(){document.documentElement.style.visibility='visible';document.documentElement.classList.add('pmt-access-ready')}
  async function boot(){if(resolved)return;var token=sessionStorage.getItem('pmt-admin-token');if(!token){deny();return}var u=user();
    // Do not blank the entire page while the permission API is round-tripping. The server still
    // enforces authorization; cached identity lets the UI paint immediately and the fresh check follows.
    if(u){reveal();filterNavigation(u)}
    ...
    u.permissions=perms(u);sessionStorage.setItem('pmt-admin-user',JSON.stringify(u));window.PMT_ADMIN_USER=u;var p=perms(u),owner=String(u.role)==='Owner',current=pageName(location.pathname)||'dashboard',need=PAGE[current];
    if(!owner&&(current==='index'||!need||!ok(p,need))){var target=first(p);if(target)location.replace('/admin/'+target);else deny();return}
    filterNavigation(u);ensureObserver(u);resolved=true;reveal();
  }
