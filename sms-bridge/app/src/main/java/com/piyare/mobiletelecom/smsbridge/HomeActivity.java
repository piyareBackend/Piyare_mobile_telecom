...
public class HomeActivity extends Activity {
    ...
    private LinearLayout card(String title,String sub,String action,View.OnClickListener click,boolean primary){LinearLayout c=new LinearLayout(this);c.setOrientation(LinearLayout.VERTICAL);c.setPadding(dp(18),dp(18),dp(18),dp(18));c.setBackground(bg(primary?(theme().equals("blueprint")?"#0B3558":theme().equals("paper")?"#2F2A20":"#111827"):surface(),16));c.setElevation(dp(2));TextView h=text(title,20,primary?Color.WHITE:textColor());h.setTypeface(Typeface.DEFAULT,Typeface.BOLD);c.addView(h);TextView p=text(sub,13.5f,primary?Color.rgb(220,228,236):mutedColor());p.setPadding(0,dp(6),0,dp(14));c.addView(p);TextView b=text(action,13,primary?Color.rgb(17,24,39):Color.WHITE);b.setTypeface(Typeface.DEFAULT,Typeface.BOLD);b.setGravity(Gravity.CENTER);b.setPadding(dp(12),dp(10),dp(12),dp(10));b.setBackground(bg(primary?"#FFFFFF":"#2563EB",11));b.setOnClickListener(click);c.addView(b,new LinearLayout.LayoutParams(-1,dp(42)));return c;}
    private void styleLogo(ImageView v){v.setImageResource(R.drawable.logo);v.setScaleType(ImageView.ScaleType.CENTER_INSIDE);v.setBackground(bg("#FFFFFF",14));v.setClipToOutline(true);}
    private TextView iconButton(String glyph,String label,View.OnClickListener click){TextView b=text(glyph,22,textColor());b.setGravity(Gravity.CENTER);b.setContentDescription(label);b.setBackground(bg(theme().equals("blueprint")?"#DCEBFA":theme().equals("paper")?"#E9E1CF":"#FFFFFF",14));b.setElevation(dp(1));b.setOnClickListener(click);return b;}
