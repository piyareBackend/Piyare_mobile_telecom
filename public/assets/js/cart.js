...
(function(){
  ...
  async function addToCart(id,variantId='',quantity=1){
    let p=getProduct(id);if(!p){await ensureProducts();p=getProduct(id);}if(!p)return false;
    const v=Array.isArray(p.variants)?p.variants.find(x=>String(x.id)===String(variantId)):null;
    const stock=Math.max(0,Number(v?.stock??p.stock??0));if(stock<=0)return false;
    const item=find(id,v?String(v.id):'');const qty=Math.max(1,Math.min(99,Number(quantity)||1));
    if(item)item.qty=Math.min(Number(item.qty||0)+qty,stock);else cart.push({...p,variantId:v?String(v.id):'',variantName:v?.name||'',price:Number(v?.price??p.price??0),stock,qty:Math.min(qty,stock)});
  ...
  function renderCart(){
    const count=document.getElementById('cartCount');if(count)count.textContent=String(cart.reduce((s,i)=>s+Math.max(0,Number(i.qty)||0),0));
    const body=document.getElementById('cartBody'),foot=document.getElementById('cartFoot');if(!body||!foot)return;
    ...
    const sub=subtotal(),discount=appliedCoupon?(appliedCoupon.type==='flat'?Math.min(Number(appliedCoupon.value||0),sub):Math.min(Math.round(sub*Number(appliedCoupon.value||0)/100),sub)):0,total=Math.max(0,sub-discount);
  ...
  function observeProductGrid(){const grid=document.getElementById('productGrid');if(!grid||grid.dataset.pmtObserved)return;grid.dataset.pmtObserved='1';const observer=new MutationObserver(enhanceProductButtons);observer.observe(grid,{childList:true,subtree:true});enhanceProductButtons();}
  window.addToCart=addToCart;window.removeFromCart=removeFromCart;window.changeCartQty=changeQty;window.getCartItems=()=>cart.slice();window.getCartSubtotal=subtotal;
  ...
  if(!document.getElementById('productGrid'))ensureProducts();
